...
public class ClientHelper {
	...
	public SampleOrg getSamleOrg()
		...
		SampleOrg sampleOrg = clientConfig.getIntegrationTestsSampleOrg("peerOrg2");
	...
	public SampleOrg setPeerAdmin(SampleStore sampleStore, SampleOrg sampleOrg)
		...
		String str1=clientConfig.getBlockChainKeyPath()+"/crypto-config/peerOrganizations/"+sampleOrgDomainName+format("/users/Admin@%s/msp/keystore", sampleOrgDomainName);
		String str2=clientConfig.getBlockChainKeyPath()+"/crypto-config/peerOrganizations/"+sampleOrgDomainName+ format("/users/Admin@%s/msp/signcerts/Admin@%s-cert.pem", sampleOrgDomainName, sampleOrgDomainName);
...
//	}
	...
	private Channel getChannel(SampleOrg sampleOrg, HFClient client)
		...
		for (String peerName : sampleOrg.getPeerNames()) {
			...
			Properties peerProperties = clientConfig.getPeerProperties(peerName, sampleOrg);
		...
		for (String eventHubName : sampleOrg.getEventHubNames()) {
			EventHub eventHub = client.newEventHub(eventHubName, sampleOrg.getEventHubLocation(eventHubName),
					clientConfig.getEventHubProperties(eventHubName, sampleOrg));
...
//		String chaincodename=clientConfig.net_work_name+"-"+clientConfig.FOO_CHANNEL_NAME+"-"+clientConfig.CHAIN_CODE_NAME;
		String chaincodename=clientConfig.CHAIN_CODE_NAME;
		return ChaincodeID.newBuilder().setName(chaincodename).setVersion(CHAIN_CODE_VERSION).setPath(CHAIN_CODE_PATH)
				.build();
	}
}
